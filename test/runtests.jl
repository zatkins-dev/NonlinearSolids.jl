...
@testset verbose = true "NonlinearSolids.jl" begin
  ...
  @testset verbose = true "ShapeFunctions" begin
    points = range(-1, 1, 10001)
    fns_order_1(ξ) = 0.5 * [1 - ξ, 1 + ξ]
    dfns_order_1(ξ) = 0.5 * [-1, 1]
    fns_order_2(ξ) = [-0.5 * ξ * (1 - ξ), (1 - ξ) * (1 + ξ), 0.5 * ξ * (1 + ξ)]
    dfns_order_2(ξ) = [ξ - 0.5, -2ξ, ξ + 0.5]

    @testset verbose = true "Lagrange - order $(p)" for p in 1:3
      @testset verbose = true "Chebyshev 2nd Type" begin
      ...
      @testset verbose = true "Equidistant" begin
        fns_order_3(ξ) = 9 / 16 * [
          -(1 - ξ) * (1 / 3 + ξ) * (1 / 3 - ξ),
          3 * (1 - ξ) * (1 + ξ) * (1 / 3 - ξ),
          3 * (1 - ξ) * (1 + ξ) * (1 / 3 + ξ),
  ...
  @testset verbose = true "Quadrature" begin
    @testset verbose = true "GaussQuadrature" begin
      @testset verbose = true "integrate" begin
        # integrate polynomial with order 2N-1 exactly
        q = gauss_quadrature(Val(2))
        f1(ξ) = ξ^3 - 2ξ^2 + 3ξ - 4
        @test integrate(q, f1) ≈ -28 / 3
        q = gauss_quadrature(Val(3))
        f2(ξ) = ξ^5 - 2ξ^4 + 3ξ^3 - 4ξ^2 + 5ξ - 6
        @test integrate(q, f2) ≈ -232 / 15
        q = gauss_quadrature(Val(4))
        f3(ξ) = ξ^7 - 2ξ^6 + 3ξ^5 - 4ξ^4 + 5ξ^3 - 6ξ^2 + 7ξ - 8
        @test integrate(q, f3) ≈ -776 / 35
      end
    end
  end
